...
impl HoleList {
    ...
    unsafe fn new(hole_addr: *mut u8, hole_size: usize) -> Self {
        ...
        Self {
            first: Hole {
                size: 0,
                next: Some(unsafe { NonNull::new_unchecked(ptr) }),
            },
...
fn check_merge_bottom(node: NonNull<Hole>, bottom: *mut u8) -> NonNull<Hole> {
    if bottom.wrapping_add(size_of::<Hole>()) > node.as_ptr().cast::<u8>() {
        ...
        make_hole(bottom, size)
    } else {
        node
    }
}

fn make_hole(addr: *mut u8, size: usize) -> NonNull<Hole> {
    unsafe {
...
// reference: https://github.com/esp-rs/esp-alloc
...
unsafe impl GlobalAlloc for LinkedListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        self.heap
            ...
            .ok()
            .map_or(ptr::null_mut(), |alloc| alloc.as_ptr())
    }
